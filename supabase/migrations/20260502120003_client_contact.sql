-- Add contact fields to client_businesses for the engagement contact
-- (separate from primary_owner_name, which is the named owner of the
-- business — usually the same person, but not always). Displayed on the
-- clients list row.
...
-- Re-declare the demo seed RPC so new Peter Smith demo cases populate
-- both fields. Existing already-seeded demo rows keep contact_name=null
-- and rely on the display-time fallback to primary_owner_name. Idempotent
-- — the function body short-circuits when a demo case already exists.
...
begin
  ...
  insert into client_businesses (
    ...
    primary_owner_name, contact_name, contact_title, created_via
  ) values (
    v_advisor_id, v_firm_id, 'Precision Auto Services', 'precisionauto.com',
    'Peter Smith', 'Peter Smith', 'Owner', 'advisor_manual'
  ...
  -- 3) valuation_snapshot — RISR-canonical numbers
